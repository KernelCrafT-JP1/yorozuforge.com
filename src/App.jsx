...
export function App() {
  ...
  return <>
    ...
    <main id="main">
      ...
      <section className="workshop-section" id="workshop" aria-labelledby="workshop-title"><div className="workshop-heading"><p className="eyebrow">工房について</p><h2 id="workshop-title">アイデアを叩いて、<br />削って、形にする。</h2></div><div className="workshop-copy"><p>YOROZU FORGE（ヨロズフォージ）は、アイデアを叩いて、削って、形にするデジタル工房です。</p><p>フォント、デザイン素材、ソフトウェア。<br />ジャンルを決めず、「こんなの欲しい」から制作を始めます。</p><p>作って、試して、壊して、また作る。<br />そうして出来上がったものを、ひとつずつ世に送り出しています。</p><p>何が出てくるかは、その時次第。<br />デジタルのよろず鍛冶場、YOROZU FORGEです。</p><a className="text-link" href={boothUrl} target="_blank" rel="noreferrer">BOOTHで制作物を見る<ArrowRight size={22} aria-hidden="true" /><span className="sr-only">（新しいタブで開きます）</span></a></div><div className="profile-inner"><div><p className="profile-label">代表</p><h3>冨田 和臣</h3></div><div><p>Web・クラウド領域のエンジニアとして、システム開発・運用、AWS/Azure環境の構築、CI/CD導入などに従事。</p><p>現在はYOROZU FORGEとして、Web制作、業務改善・自動化、デジタルプロダクトの制作を行っています。</p></div></div></section>
