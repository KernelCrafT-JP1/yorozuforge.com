...
function Brand({ onClick }) {
  return <a className="brand" href="#top" onClick={onClick} aria-label="YOROZU FORGE トップへ"><img src="./assets/yorozu-logo.png" alt="" width="600" height="600" /><span>YOROZU FORGE <small>（ヨロズフォージ）</small></span></a>;
}
...
export function App() {
  ...
  const nav = <><a href="#services" onClick={() => setMenuOpen(false)}>相談・開発</a><a href="#workshop" onClick={() => setMenuOpen(false)}>工房について</a><a href={boothUrl} target="_blank" rel="noreferrer" onClick={() => setMenuOpen(false)}>BOOTH<span className="sr-only">（新しいタブで開きます）</span></a><button className="nav-contact" onClick={contact}>お問い合わせ</button></>;
  ...
  return <>
    ...
    <main id="main">
      ...
      <section className="manifesto" id="workshop" aria-label="工房のものづくり"><h2><span>作って、試して、</span><span>壊して、また作る<em>。</em></span></h2></section>
      <section className="services" id="services" aria-labelledby="services-title"><h2 id="services-title">仕事の「<em>欲しい</em>」も、ここから。</h2><div className="service-columns"><article><h3>IT相談</h3><p>困りごとを整理し、<br />進め方を一緒に<br className="mobile-break" />考えます。</p></article><article><h3>受託開発</h3><p>仕事に合わせて、<br />必要な仕組みを<br className="mobile-break" />つくります。</p></article></div><div className="workshop-copy"><p>フォント、デザイン素材、ソフトウェア。<wbr />ジャンルを決めず、「こんなの欲しい」から制作を始めます。<br className="desktop-break" />そうして出来上がったものを、ひとつずつ世に送り出しています。</p><a className="text-link" href={boothUrl} target="_blank" rel="noreferrer">BOOTHで制作物を見る<ArrowRight size={22} aria-hidden="true" /><span className="sr-only">（新しいタブで開きます）</span></a></div><details className="profile"><summary>工房と、つくる人について<span className="details-indicator" aria-hidden="true"><ArrowRight size={18} /></span></summary><div className="profile-inner"><div><p className="profile-label">代表</p><h3>冨田 和臣</h3><p>何が出てくるかは、その時次第。<br />デジタルのよろず鍛冶場、YOROZU FORGEです。</p></div><div><p>Web・クラウド領域のエンジニアとして、システム開発・運用、AWS/Azure環境の構築、CI/CD導入などに従事。</p><p>現在はYOROZU FORGEとして、Web制作、業務改善・自動化、デジタルプロダクトの制作を行っています。</p><p className="profile-note">ご相談内容に合わせて、対応範囲や進め方を一緒に確認します。</p></div></div></details></section>
